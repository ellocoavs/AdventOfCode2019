...
namespace Day_5
{
    class Program
    {
        ...
        static int Compute (int[] opcodes, int noun, int verb) 
        {
            opcodes[1]= noun;
            opcodes[2]= verb;
            for (int position = 0; ; position += 1)  //edit to do +1 and do an extra +3 if needed
            {
                ...
                else if (opcodes[position] == 3) //only one parameter, no +3 needed
                {
                 
                    //Console.WriteLine("Result stored at position: " + position + " is: " + opcodes[position+3]);
                }
                else if (opcodes[position] == 4)
                {
